...
public abstract class EnemyParent {
	...
	public int checkCollision(int marioX, int marioY, int goombaX, int goombaY) {
		int value = 0;
		if (Math.abs(marioX - goombaX) <= 49 && Math.abs(marioY - goombaY) <= 60 && Mplayer.ySpeed > 0
				&& (Mplayer.ySpeed < 37 || Mplayer.ySpeed > 48) && Mplayer.ySpeed >= 33) {
			value = 1;
		...
		} else if (marioY <= goombaY)// condition for side collision
		{
			if (Math.abs(marioY - goombaY) < 50 && Math.abs(marioX - goombaX) <= 50) {
				if (!Mplayer.isStar) {// System.out.println("mY"+marioY+"gY"+goombaY+"mX"+marioX+"gX"+goombaX+"sp"+Mplayer.ySpeed);
					value = 2;
				} else {
					value = 3;
