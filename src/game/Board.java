...
public class Board extends JPanel implements gameConstants {
	...
	private void loadImage() {
		try {// image =
				// ImageIO.read(Board.class.getResource("hdMariomain.jpg")).getSubimage(camera.getX(),2*IMAGE_HEIGHT,IMAGE_WIDTH,IMAGE_HEIGHT);

			image = ImageIO.read(Board.class.getResource("hdMariomain.jpg")).getSubimage(camera.getX(),
					2 * IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_HEIGHT);
			// try {image =
			// ImageIO.read(Board.class.getResource("hdMariomain.jpg")).getSubimage(1046,camera.getX(),IMAGE_WIDTH,IMAGE_HEIGHT);
		...
		this.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if (!Mplayer.isReachedEnd) {
					...
					if (e.getKeyCode() == KeyEvent.VK_DOWN && (Mplayer.x - pipeImages.x1) < 25
							&& (Mplayer.x - pipeImages.x1) > -35) {// BackGroundMusic.playPipe(); music
	...
	public void paintComponent(Graphics g) {
		...
		else { // BackGroundMusic.mp31.stop();
